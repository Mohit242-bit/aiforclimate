...
def export_results():
    ...
    return jsonify({
        ...
        'zones_aqi': zone_aqi,
        'zone_traffic': results['zones'],
        'od_travel_times': results['od_travel_times'][:10],  # Top 10 OD pairs
    }), 200
...
def get_segment_details(segment_id):
    ...
    Returns:
        Segment properties, current state, emissions
    """
    global network, simulator, emissions_model
    ...
    seg_results = simulator.get_segment_results(segment_id)
    seg_emissions = emissions_model.compute_segment_emissions(segment_id)
    
    return jsonify({
        'segment_id': segment_id,
        'properties': seg,
        'current_state': seg_results,
        'emissions': seg_emissions,
    }), 200

@corridor_api.route('/zone/<zone_id>', methods=['GET'])
def get_zone_details(zone_id):
    """
    Get aggregated information about a zone.
    
    Returns:
        Zone traffic statistics and AQI
    """
    global network, simulator, emissions_model
    ...
    zone_traffic = simulator.get_zone_results(zone_id)
    zone_aqi = emissions_model.compute_zone_aqi(zone_id)
    health_impact = emissions_model.compute_health_impact(zone_id)
    
    segments = network.get_segments_in_zone(zone_id)
    intersections = network.get_intersections_in_zone(zone_id)
    
    return jsonify({
        'zone_id': zone_id,
        'traffic_stats': zone_traffic,
        'aqi_data': zone_aqi,
...
@corridor_api.route('/intervention', methods=['POST'])
def apply_intervention():
    """
    Apply a single intervention and get impact.
    ...
    Returns:
        Intervention details and estimated impact
    """
    global intervention_engine, simulator
    
    if intervention_engine is None:
        return jsonify({'error': 'Intervention engine not initialized'}), 500
    
    data = request.get_json()
    interv_type = data.get('type')
    
    # Apply intervention
    if interv_type == 'add_lanes':
        result = intervention_engine.add_lanes(
            data['segment_ids'],
            data.get('num_lanes', 1)
        )
    elif interv_type == 'signal_timing':
        result = intervention_engine.modify_signal_timing(
            data['intersection_ids'],
            data.get('cycle_time'),
            data.get('green_time')
        )
    elif interv_type == 'truck_ban':
        result = intervention_engine.truck_ban(
            data['segment_ids'],
            data.get('time_window')
        )
    else:
        return jsonify({'error': f'Unknown intervention type: {interv_type}'}), 400
    
    # Run simulation with intervention
    intervention_id = result['intervention_id']
    scenario_results = simulator.run_simulation(f'with_{intervention_id}')
    
    return jsonify({
        'intervention': result,
        'simulation_results': {
            'total_vehicles': scenario_results['total_vehicles'],
            'zones': scenario_results['zones'],
        }
    }), 200

@corridor_api.route('/interventions/active', methods=['GET'])
def get_active_interventions():
    """
    Get list of currently active interventions.
...
def reset_interventions():
    """
    Reset all interventions and return to baseline.
...
@corridor_api.route('/network/validate', methods=['GET'])
def validate_network():
    """
